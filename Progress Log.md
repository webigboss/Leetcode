...
416|Partition Equal Subset Sum|Medium|8/25/2019|Inspired only by idea(developed code independently)|DP (0/1 knapsack problem)|||DP: O(n*m); SC:(n*m), n: nums.Length, m: sum/2;|"define a boolean 2D array dp, that dp[i,j] means if the first i elements in the array nums can have a subset with a sum = sum(nums) / 2; transition function: dp[i,j] = dp[i - 1, j] || (j - nums[i] <  0 ? false : dp[i - 1, j - nums[i]])"
417|Pacific Atlantic Water Flow|Medium|8/25/2019|Yes|DFS (iterative) / BDS (to be implemented)|||DFS: O(n*m), SC: O(n*m)|DFS: 1. define a 2d array to save the state of each element, 0: unvisited, 1: cannot flow to both ocean, 2: can flow to Pacific, 3: can flow to Atlantic, 4: can flow to both oceans; 2. use stack to do DFS iteratively; 3. make sure to have a 2d array to record visited element; 4. logic to update the state continously, e.g. when current state indicate it can flow to pacific, and current element's x or y index is the right and bottom boarder. then update state to 4. if current state is 4. then update the starting element to 4 too etc.
